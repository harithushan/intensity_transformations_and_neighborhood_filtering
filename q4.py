...
def main():
    ...
    h, s, v = cv2.split(hsv)
    cv2.imwrite("docs/q4_images/output/q4_spider_h.png", h.astype(np.uint8))
    cv2.imwrite("docs/q4_images/output/q4_spider_s.png", s.astype(np.uint8))
    cv2.imwrite("docs/q4_images/output/q4_spider_v.png", v.astype(np.uint8))
    x = np.arange(256, dtype=np.float32)
    y = vibrance_s_curve(x, a=A, sigma=SIGMA).astype(np.float32)
    plot_transfer(x, y, f"Q4 Vibrance Transform (a={A:.2f}, sigma={SIGMA})", "docs/q4_images/output/q4_spider_transfer.png")
